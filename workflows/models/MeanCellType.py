...
from src.CellTypeModel.model import CellTypeModel
...
sample_ids = training_data["barcode"]
sample_ids = [s.split("_")[1] for s in sample_ids]


regressor = CellTypeModel(**param)
regressor.fit(training_data["X"], training_data["y"][:, selected_genes_bool], sample_ids)
